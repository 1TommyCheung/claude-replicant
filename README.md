...
Claude Replicant captures a selected project and its locally available Claude Code state into a self-describing capsule with an immutable, hash-verified canonical payload. A project may be a Git repository or an ordinary folder. Both modes include sessions, memory, subagents, file history, plans, tasks, skills, commands, plugins, hooks, settings, and project files; Git-backed mode additionally preserves and verifies the repository's Git state. Operational plans, receipts, and future derivatives may be added only inside their dedicated capsule subfolders.
...
- **A canonical project record:** captured bytes, applicable Git state, provenance, inventories, and integrity hashes remain the source of truth.
...
- ordinary folders with no `.git` entry, without requiring Git during capture, planning, or restore;
- normal, self-contained `.git/` repositories with tracked, staged, unstaged, untracked, ignored/generated, refs, objects, and index state;
- strict Git requirements for sources that contain `.git`, with no silent downgrade to folder mode;
...
- remapping of Claude Code’s path-derived project key to the restored project path;
...
- post-restore project and Claude-state verification, plus Git verification when applicable, with a restore receipt.
...
- Git `>=2.39.0 <3.0.0` only for a source or capsule with applicable Git state
...
Source mode is automatic. If `<project>/.git` exists, Claude Replicant uses `git-repository` mode and requires the supported Git version. Otherwise it uses `folder` mode and records Git as `not-applicable`; Git is not invoked during capture, planning, or restore. When `--claude-home` is omitted, capture uses `CLAUDE_CONFIG_DIR` and then `~/.claude`. When `--claude-destination` is omitted, planning uses `<new-project>.claude-home`. Plan and receipt output paths outside the capsule are rejected.
...
Example goal: preserve a project and all locally stored Claude Code sessions in one movable capsule. If the project is a Git repository, preserve and verify its Git state too.

Prompt for Codex or Claude Code:

```text
Use the Claude Replicant plugin to back up the project at
/path/to/my-project into the capsule store /path/to/backup/replicant.

First run a read-only preview. Report the resolved project, detected source type,
Git state or not-applicable status, project entry count, Claude-state entry count,
session count, exclusions, and readiness findings. Do not create the capsule until
I explicitly reply "approve capture".
...
### Use case 1b: back up a project that does not use Git

Use the same command and prompt. Do not run `git init` merely for Claude Replicant. When `/path/to/my-project/.git` is absent, the preview reports `sourceKind: folder` and Git `not-applicable`. The confirmed capsule still contains the project files, Claude sessions, memory, agents, plugins, settings, reports, and isolated restore state. Planning and restore also run without Git on the destination computer.

If `.git` is present, Claude Replicant intentionally selects `git-repository` mode and requires a compatible Git installation so repository state cannot be discarded silently.
...
Install Claude Replicant on the destination computer, copy the complete capsule there, and choose two new paths that do not already exist: one for the project and one for the isolated Claude home.
...
to the new project path /path/to/restored-project and the new isolated Claude
...
After approval, execute the plan, require project/Claude-state/native-session
verification to pass and Git verification when applicable, and give me the JSON,
Markdown, and HTML restore reports plus the exact CLAUDE_CONFIG_DIR and resume
commands.
```
...
Launch Claude Code from the restored project using the isolated restored state:
...
Capture, integrity validation, corruption refusal, dry-run planning, isolated restore, and post-restore verification for a selected Git repository or ordinary folder plus local Claude Code sessions, memory, agents, and configuration.
...
The synthetic fixtures cover Git-repository and Git-free folder capture, sessions, memory, subagents, skills, agents, static HTML reporting, project-key remapping, isolated Claude-home restoration, validation, corruption refusal, applicable/not-applicable Git verification, credential-store exclusion, live writes, symlinks, hardlinks, ignored files, and Unicode paths.
